...
Para ello, el sistema lista todos los hostales registrados y el usuario selecciona uno.
A continuación, el sistema lista los empleados registrados en el sistema que todavía no
han sido asignados a dicho hostal
y el usuario selecciona uno, junto con el cargo que desempeñará en dicho hostal 
(administración, limpieza, recepción, infraestructura).
...
void cu_asignarEmpleadoAHostal(){
    ...
    while (seguir == true){    
        ...
        set<DTEmpleado*> empleados = iusuario->obtenerEmpleadosNoRegistrados(h);
